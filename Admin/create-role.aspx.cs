    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                {
                    if (CreateUser.CheckAccess(conSQ, pageName, "Add", Request.Cookies["nt_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conSQ = new SqlConnection(ConfigurationManager.ConnectionStrings["conSQ"].ConnectionString);
            if (CreateUser.CheckAccess(conSQ, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["nt_aid"].Value))
            {
